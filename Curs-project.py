...
def statistic():
    print('Смотрите статистику в файле statictics')
    with open('statistic.txt', "w") as f:
        for player, val in statistics.items():
            f.write(f'{player} \n')
            for i in val:
                f.write(f'{i[0]} \n')
...
def menu():
    print('Для выбора пункта меню введите указанную цифру')
    print('[0]Игрок1- Игрок2')
    print('[1]Игрок1- Компьютер')
    print('[2]Статистика')
    print('[3]Выход')
    choise_menu.clear()
    try:
        choise_menu.append(int(input()))
    except ValueError:
        print('Ошибка ввода')
        menu()
    else:
...
#     players.append(Player(name='Player2', is_ai=False, auto_ship=True, skill=1))
# elif choise[0] == 1:
#     players = []
#     players.append(Player(name='Player1', is_ai=False, auto_ship=True, skill=1))
...
def Main_Game():
    first = 0
    second = 0
    statistics.setdefault('player1', 0)
    statistics.setdefault('player2', 1)
    while True:
        game.status_check()
        ...
        if game.status == 'game over':
            ...
            statistics.setdefault(game.next_player.name, str(first))
            print('{} выиграл матч! Поздравления!'.format(game.current_player.name))
            statistics.setdefault(game.current_player.name, str(second))
            game.status = 'prepare'
            game.status_check()
...
while True:
    ...
    if choise_menu[0] == 0:
        ...
        Main_Game()
        choise_menu.clear()
    elif choise_menu[0] == 1:
        players = []
        players.append(Player(name='Player1', is_ai=False, auto_ship=True, skill=1))
        players.append(Player(name='Computer', is_ai=True, auto_ship=True, skill=1))
        Main_Game()
        choise_menu.clear()
    elif choise_menu[0] == 2:
        statistic()
        choise_menu.clear()
    elif choise_menu[0] == 3:
        choise_menu.clear()
        sys.exit()
